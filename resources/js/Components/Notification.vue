        ...
        <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" 
            ...
            <div class="flex items-center">
                ...
                <div v-else class="py-4 text-white text-sm font-medium">
                    <span v-if="Object.keys($page.props.errors).length === 1">No guardado. Hubo un error en el registro.</span>
                    <span v-else>No guardado. Hubieron {{ Object.keys($page.props.errors).length }} errores en el registro.</span>
                </div>
            </div>
            <button type="button" class="group mr-2 p-2" @click="show = false">
